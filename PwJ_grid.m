...
grid_PwJ_MLE = zeros(number_of_images,number_of_words);
grid_PwJ_GRE = zeros(number_of_images,number_of_words); % GRE: General Relative Frequency
...
%         num_of_blobs_in_image_i = length(find(mapping_of_reg_to_images == image_i_number));
        ...
        grid_PwJ_MLE(i,j) = is_word_j_included_in_image_i_captions / (num_of_word_j_in_image_i + num_of_blobs_in_image_i);
...
%         sum(documentwords(documentwords == j))
        ...
        grid_PwJ_GRE(i,j) = occur_of_word_j_in_T / T;
    end
